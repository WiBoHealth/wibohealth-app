...
// Version 8.1.0 - FINAL FIX - Arabic & English Only 🇪🇬🇬🇧✅

const CACHE_NAME = 'wibo-health-v8.1.0-final';
const RUNTIME_CACHE = 'wibo-health-runtime-v8.1.0';
const IMAGE_CACHE = 'wibo-health-images-v8.1.0';
const API_CACHE = 'wibo-health-api-v8.1.0';
const PRECACHE_URLS = [
  '/',
  '/index.html',
  '/foods.html',
...
self.addEventListener('install', event => {
  console.log('✅ Service Worker: Installing v8.1.0 - FINAL FIX...');
...
self.addEventListener('activate', event => {
  console.log('🔄 Service Worker: Activating v8.1.0 - FINAL FIX! ✅');
    ...
    .catch(error => {
      ...
      return caches.match(request)
        .then(cachedResponse => {
          ...
          // إذا كانت صفحة HTML وغير موجودة، جرب index.html
          if (request.url.includes('.html') || request.url.endsWith('/')) {
            return caches.match('/index.html').then(indexResponse => {
              if (indexResponse) {
                return indexResponse;
              }
              // لو ما فيه index.html، أرجع Response مناسبة
          ...
          // إذا فشل كل شيء، أرجع Response مناسبة
    ...
    .then(cachedResponse => {
      ...
      return fetch(request)
        ...
        .catch(error => {
          ...
          // أرجع Response مناسبة بدلاً من throw error
          return new Response('Network error occurred', {
            status: 503,
            statusText: 'Service Unavailable',
            headers: new Headers({
    ...
    .then(cachedResponse => {
      const fetchPromise = fetch(request)
        ...
        .catch(error => {
          ...
          // إذا كان في Cache، استخدمه، وإلا أرجع خطأ مناسب
          if (cachedResponse) {
            return cachedResponse;
          }
          // أرجع Response فارغة بدلاً من undefined
          return new Response('Network error occurred', {
            status: 503,
            statusText: 'Service Unavailable',
            headers: new Headers({
